            ...
            {jitsiRoom && (
                <div className="fixed inset-4 md:inset-10 z-[10000] bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-gray-200 animate-in zoom-in-95">
                    ...
                    <iframe 
                        ...
                        src={`https://senfcall.de/${jitsiRoom}#config.startWithVideoMuted=${callType === 'audio'}&config.prejoinConfig.enabled=false&config.prejoinPageEnabled=false&userInfo.displayName="%20${encodeURIComponent(user?.fullName || user?.email?.split('@')[0] || 'User')}%20"`}
