...
export const dealService = {
    ...
    create: async (deal: any) => {
        // Resolve customer name to ID if provided as `customer`
        let customerId = deal.customerId;
        if (!customerId && deal.customer) {
            // Look up customer by company_name (case-insensitive)
            const { data: custData, error: custErr } = await supabase
                .from('customers')
                .select('id')
                .eq('company_name', deal.customer)
                .single();
            if (custErr) {
        ...
        // Parse expected revenue from possible fields
        ...
        // Build payload – include customer_id only when we have one
        const payload: any = {
            title: deal.title,
        ...
        console.log('Deal insert payload:', payload);
        ...
        // Email notification
        if (data && data.length) {
            try { await sendActivityMail('deals', data[0]); }
            catch (mailError) { console.error('Failed to send deal creation email:', mailError); }
        }
...
export const offerService = {
    ...
    create: async (offer: any) => {
        ...
        console.log('Offer insert payload:', mapped);
        ...
        if (data && data.length > 0) {
            try {
                await sendActivityMail('offers', data[0]);
            } catch (mailError) {
                console.error('Failed to send offer creation email:', mailError);
            }
        }
        return data ? data[0] : null;
    },
