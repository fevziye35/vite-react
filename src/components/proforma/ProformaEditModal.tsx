...
export default function ProformaEditModal({ onClose, onSave }: ProformaEditModalProps) {
    ...
    const [date, setDate] = useLocalStorage('proforma_date', '06/23/2026');
    const [invoiceNumber, setInvoiceNumber] = useLocalStorage('proforma_invoiceNumber', '#0001');
    const [taxRate, setTaxRate] = useLocalStorage('proforma_taxRate', 10);

    // Header contact
    // Seller contact (Editable as requested)
    const [phone, setPhone] = useLocalStorage('proforma_phone', '123-456-7890');
    const [address, setAddress] = useLocalStorage('proforma_address', '123 Anywhere St., Any City, ST 12345');
    const [email, setEmail] = useLocalStorage('proforma_email', 'hello@reallygreatsite.com');
    const [website, setWebsite] = useLocalStorage('proforma_website', 'www.reallygreatsite.com');

    // Payment details
    const [accName, setAccName] = useLocalStorage('proforma_accName', 'MAKFA GIDA ÜRÜNLERİ SANAYİ TİCARET ANONİM ŞİRKETİ');
    const [accNumber, setAccNumber] = useLocalStorage('proforma_accNumber', 'TR030020500009075839900101');
    const [bankName, setBankName] = useLocalStorage('proforma_bankName', 'KUVEYT TÜRK KATILIM BANKASI A.Ş');
    const [bankAddress, setBankAddress] = useLocalStorage('proforma_bankAddress', 'İzmir Turkey');
    const [bankSwift, setBankSwift] = useLocalStorage('proforma_bankSwift', 'KTEFTRIS');

    const [items, setItems] = useLocalStorage<InvoiceItem[]>('proforma_items', [
        { id: '1', particulars: 'Email Management', hours: 5, rate: 20.00 },
        { id: '2', particulars: 'Social Media Management', hours: 7, rate: 20.00 },
    ]);

    const [customColumns, setCustomColumns] = useLocalStorage<CustomColumn[]>('proforma_custom_columns', []);
    ...
    const subtotal = items.reduce((sum, item) => {
        ...
        // Add numeric custom column values to the item amount
        customColumns.forEach(col => {
    ...
    const handleToggleEdit = () => {
        if (isEditMode && onSave) {
            // Determine primary product from items
            const primaryProduct = items[0]?.particulars || 'Yeni Proforma';
            const formattedTotal = `$${total.toFixed(2)}`;
            
            onSave({
                product: primaryProduct,
                price: formattedTotal,
                recipient: 'MAKFA GIDA...' // Hardcoded for now based on UI
            });
        }
                    ...
                    <div className="mt-8 flex flex-col items-center">
                        ...
                        <div className="max-w-2xl w-full flex flex-col items-center">
                            ...
                            <div className="bg-[#1e3a5f] text-white flex justify-between items-center px-6 py-2.5 mt-2 w-full rounded-sm">
                                ...
                                <h2 className="text-2xl tracking-[0.2em] uppercase font-bold text-center flex-1">PROFORMA</h2>
                    ...
                    <table className="w-full mt-4 border-collapse text-[10px]">
                        <thead>
                            <tr className="bg-[#1e3a5f] text-white">
                                ...
                                <th className="py-2 px-3 text-center border-r border-white/10 w-20 text-white font-bold uppercase tracking-wider">Hours</th>
